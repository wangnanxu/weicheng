			...
			return {
				...
				compile: function(tElement, tAttrs, tTransclude) {
					...
					var template = angular.element([
						...
						'<ion-scroll delegate-handle="alphaScroll">', // class="has-footer"
						...
						'<li ng-click="alphaScrollGoToList(\'index_{{letter}}\')" ng-repeat="letter in alphabet">{{letter}}</li>', //{{$index%2 == 0 ? letter: "&bull;"}}
						'</ul>',
						'</ion-list>'
					].join(''));
